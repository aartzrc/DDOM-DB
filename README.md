# DDOM Database Processors  

`DBProcessor` provides DDOM style access to a typical database.  
`DDOMDBProcessor` uses the `ddomdb.sql` schema and can store/retrieve any DDOM nodes.  
